...
object LabeledSequenceCodecs {
  ...
  def decodeAndApplyBioLabels[A <: LabelTarget](
    jsonRep: Json,
  ...
  implicit def decodeLabelSpan: Decoder[LabelSpan] = Decoder
    .decodeTuple3[Label, Int, Int]
    .map { t => LabelSpan(t._1, t._2, t._3) }

  implicit def encodeLabelSpan: Encoder[LabelSpan] = Encoder
    .encodeTuple3[Label, Int, Int]
    .contramap { ls => (ls.label, ls.begin, ls.length) }

  implicit def decodeLabelingTree: Decoder[LabelingTree] = Decoder
    .decodeTuple2[LabelSpan, Seq[Json]]
    .map { t =>
      LabelingTree(t._1, t._2.map(_.decodeOrDie[LabelingTree]()))
    }

  implicit def encodeLabelingTree: Encoder[LabelingTree] = Encoder
    .encodeTuple2[LabelSpan, Seq[Json]]
    .contramap { lTree =>
