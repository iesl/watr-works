...
class DocumentSegmenter(
  ...
  def labelTitle(): Unit = {
    ...
    val biggestLineAtBeginning = vlines.take(5)
      .filter(_.component.toText.length() > 5)
      .sortWith(_.component.height > _.component.height)
    biggestLineAtBeginning.foreach(node => println(node.component.toText))
    println

    //todo: why isn't this actually labeling anything
    if(biggestLineAtBeginning.headOption.isDefined) {
      println("Title candidate: " + biggestLineAtBeginning.headOption.get.component.toText)
