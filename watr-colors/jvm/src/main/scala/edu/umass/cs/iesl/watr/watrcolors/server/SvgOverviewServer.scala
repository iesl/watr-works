...
class SvgOverviewServer(
  ...
  def createView(corpusEntryId: String): List[HtmlUpdate] = {
    corpus
      ...
      .asFile
      .map({ f =>
        val corpusPath = File(corpus.config.rootDirectory).relativize(f)
        println(s"SvgOverviewServer: createView(${corpusEntryId}) path=(${corpusPath})")
        List(
          HtmlReplaceInner("#main", new html.SvgOverviewView(config).init(corpusPath.toString).toString)
        )
      }).getOrElse ({
        List(
  ...
  def onSelectBBox(artifactId: String, bbox: BBox): List[HtmlUpdate] = {
    ...
    val bboxes = getCharLevelOverlay(artifactId)

    val overlapBboxes = for (b <-bboxes if overlaps(b, bbox)) yield b
  ...
  def getCharLevelOverlay(corpusEntryId: String): List[BBox] = {
