...
// Document Segmentation Tags
// sealed trait UpLeftChar
...
trait TypeTags extends TypeTagUtils {
  ...
  // val UpLeftChar = Tag.of[UpLeftChar]
}
...
trait TypeTagCodecs {
  ...
  implicit def Enc_StringTypeTags[T]: Encoder[String @@ T] =
    Encoder.encodeString.contramap(_.unwrap)

  implicit val Enc_Str_DocumentID: Encoder[String @@ DocumentID] =
    Encoder.encodeString.contramap(_.unwrap)
  implicit val Dec_Str_DocumentID: Decoder[String @@ DocumentID] =
    Decoder.decodeString.map(DocumentID(_))

  implicit val Enc_Int_DocumentID: Encoder[Int @@ DocumentID] =
  ...
  implicit val Enc_String_EmailAddr: Encoder[String @@ EmailAddr] =
    Encoder.encodeString.contramap(_.unwrap)
  implicit val Dec_String_EmailAddr: Decoder[String @@ EmailAddr] =
    Decoder.decodeString.map(EmailAddr(_))

  implicit val Enc_String_WorkflowID: Encoder[String @@ WorkflowID] =
    Encoder.encodeString.contramap(_.unwrap)
  implicit val Dec_String_WorkflowID: Decoder[String @@ WorkflowID] =
    Decoder.decodeString.map(WorkflowID(_))

  implicit val Enc_String_CorpusPath: Encoder[String @@ CorpusPath] =
    Encoder.encodeString.contramap(_.unwrap)
  implicit val Dec_String_CorpusPath: Decoder[String @@ CorpusPath] =
    Decoder.decodeString.map(CorpusPath(_))

  implicit val Enc_String_StatusCode: Encoder[String @@ StatusCode] =
    Encoder.encodeString.contramap(_.unwrap)
  implicit val Dec_String_StatusCode: Decoder[String @@ StatusCode] =
    Decoder.decodeString.map(StatusCode(_))
  ...
  implicit val Enc_Int_AnnotationID: Encoder[Int @@ AnnotationID] =
    Encoder.encodeInt.contramap(_.unwrap)
  implicit val Dec_Int_AnnotationID: Decoder[Int @@ AnnotationID] =
    Decoder.decodeInt.map(AnnotationID(_))
